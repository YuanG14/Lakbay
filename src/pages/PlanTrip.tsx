      ...
      <form className="panel form-panel calculator-form" onSubmit={calculate}>
        ...
        <div className="calculator-section">
          ...
          <div className="field-grid three-fields">
            <label><span>One-way distance</span><div className="number-input-shell route-distance-field"><BlankableNumberInput min="0" step="0.1" value={distance} disabled={!manualDistance && routeResolved} onValueChange={setDistance}/><span className="input-affix suffix">km</span></div><button className="manual-distance-link" type="button" onClick={() => setManualDistance((c) => !c)}>{manualDistance ? 'Use automatic route instead' : 'Edit distance manually'}</button></label>
        ...
        <div className="calculator-section">
          ...
          <div className="dynamic-cost-list">{tollItems.map((item) => <div className="dynamic-cost-row" key={item.id}><div className="dynamic-cost-name"><Route size={16}/><input value={item.label} onChange={(e) => updateToll(item.id, { label: e.target.value })} aria-label="Toll name"/></div><div className="number-input-shell compact-money"><span className="input-affix">₱</span><BlankableNumberInput min="0" step="0.01" value={item.oneWayAmount} onValueChange={(value) => updateToll(item.id, { oneWayAmount: value })}/></div><button type="button" className="dynamic-remove" onClick={() => removeToll(item.id)} aria-label="Remove toll"><Trash2 size={15}/></button></div>)}</div>
        ...
        <div className="calculator-section">
          ...
          <div className="dynamic-cost-list">{expenseItems.map((item) => <div className="dynamic-cost-row expense-row" key={item.id}><div className="dynamic-cost-name">{expenseIcon(item.category)}<input value={item.label} onChange={(e) => updateExpense(item.id, { label: e.target.value })} aria-label="Expense label"/></div><select className="expense-category-select" value={item.category} onChange={(e) => updateExpense(item.id, { category: e.target.value as ExpenseCategory })}><option value="food">Food</option><option value="accommodation">Stay</option><option value="ferry">Ferry</option><option value="parking">Parking</option><option value="other">Other</option></select><div className="number-input-shell compact-money"><span className="input-affix">₱</span><BlankableNumberInput min="0" step="0.01" value={item.amount} onValueChange={(value) => updateExpense(item.id, { amount: value })}/></div><button type="button" className="dynamic-remove" onClick={() => removeExpense(item.id)} aria-label="Remove expense"><Trash2 size={15}/></button></div>)}</div>
